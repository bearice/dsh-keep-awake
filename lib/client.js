    ...
    function createSection(t) {
      function Section() {
        ...
        if (error !== '') {
          ...
          if (!state.enabled) badge = [t('badge.disabled'), styles.badgeOff]
          else if (state.active) {
            if (state.helperState === 'error') badge = [t('badge.helperError'), styles.badgeErr]
            else if (state.graceRemainingMs > 0) {
              badge = [t('badge.grace', { s: Math.ceil(state.graceRemainingMs / 1000) }), styles.badgeGrace]
            } else {
              badge = [t('badge.on'), styles.badgeOn]
            }
          } else {
